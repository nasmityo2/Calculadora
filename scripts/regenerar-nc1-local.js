 ...
 * misma variable que en Vercel) y lo persiste vía licenciaService (SQLite).
 *
 * Uso:
...
const { getOne } = require('../backend/config/db');
...
async function main() {
  ...
  const row = getOne(`SELECT valor FROM configuracion WHERE clave = 'licencia_empresa'`);
  ...
  const info = await licenciaService.activarLicencia(null, token, HWID);
  console.log(
    JSON.stringify(
      {
        ok: true,
