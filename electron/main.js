...
// Compatibilidad Electron 22 en hardware antiguo (Win 7 / GPUs sin driver moderno):
// desactivar el sandbox de GPU evita crashes del proceso GPU en equipos de 2 GB RAM.
// Debe registrarse ANTES de app.whenReady().
app.commandLine.appendSwitch('disable-gpu-sandbox');
// Solo agregar 'no-sandbox' si se reportan problemas específicos en Win 7.
...
/**
 * Carpeta de respaldos en userData. Los respaldos son copias nativas de la BD
 * SQLite (backupService → better-sqlite3 .backup()); ya no se usa pg_dump.
 */
...
/** Backend embebido (Express + SQLite). */
...
/** @type {2 | 3} — 2: licencia (instalación/renovación), 3: solo admin/moneda/empresa. */
let setupWindowStartStep = 2;
/** Primera instalación (textos de bienvenida en lugar de "renovar licencia"). */
let setupWindowFirstRun = false;
...
// ── Asistente de primera ejecución (licencia + admin + moneda + empresa) ──

function createSetupWindow(options = {}) {
  const start = Number(options.startStep);
  setupWindowStartStep = start === 3 ? 3 : 2;
  setupWindowFirstRun = Boolean(options.firstRun);
  if (setupWindowFirstRun) {
...
function registerSetupIpc() {
  ipcMain.handle('setup:get-initial-step', () => ({
    ...
    firstRun: setupWindowFirstRun,
    renewalOnly: setupWindowStartStep === 2 && !setupWindowFirstRun,
...
async function fetchSetupAdminEstado() {
  try {
    ...
    const res = await getJson('http://127.0.0.1:3000/api/setup/estado');
    if (!res.ok) return { adminPendiente: true };
    return res.body || { adminPendiente: true };
  } catch (_e) {
...
async function evaluateLicenseGate(opts = {}) {
  ...
  // Cuando el token local expiró, se excedió el período de gracia offline o se detectó
  // retroceso del reloj del sistema, intentar verificación online antes de bloquear:
  // el admin pudo haber renovado la licencia en el servidor Vercel y el cliente aún
  // tiene el token viejo (y el verify online revalida el ancla temporal local).
  if (local.state === 'expired' || local.state === 'grace_exceeded' || local.state === 'clock_rollback') {
    try {
...
function buildStartupDiagnostic(err) {
  ...
  let dbDir = '(desconocido)';
  try {
    dbDir = path.join(app.getPath('userData'), 'NexusCore_data');
  } catch (_e) {}
  return {
    message: msg,
    technicalLog:
      ...
      'Verifica que la carpeta de datos sea accesible (permisos de escritura)\n' +
      'y que el archivo nexus.db no esté bloqueado por otro proceso.'
  };
}

async function runStartupSequence() {
  updateSplashStatus('Preparando base de datos local...', 10);
...
async function handleStartupFailure(err) {
  ...
  const { response } = await dialog.showMessageBox({
    ...
    message: 'No se pudo iniciar el sistema.',
    detail: `${diag.message}\n\nPuedes reintentar el arranque o salir.`,
...
app.whenReady().then(async () => {
  ...
  console.log(`${LOG_PREFIX} Motor BD: SQLite (better-sqlite3)`);
  ...
  if (needsFirstRunSetup(app)) {
    console.log(`${LOG_PREFIX} Primera ejecución — asistente de instalación (licencia + admin + moneda + empresa).`);
    // SQLite no requiere credenciales: el config.env (JWT) se genera antes del wizard
    // para que el backend embebido arranque ya con el secreto definitivo.
    try {
      await saveUserConfigEnv(app);
      applySavedConfigToProcess(app);
      applyNexusBackupEnv();
    } catch (err) {
      console.error(`${LOG_PREFIX} No se pudo generar config.env:`, err.message);
    }
    const setupOk = await createSetupWindow({ startStep: 2, firstRun: true });
    if (!setupOk) {
