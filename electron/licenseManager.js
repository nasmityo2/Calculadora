...
async function postServer(pathSeg, body, timeoutMs = 8000) {
  return postJson(serverUrl() + pathSeg, body, timeoutMs);
}
...
function evaluate(app) {
  ...
  // Anti retroceso de reloj: si el reloj del sistema está ANTES de la última verificación
  // registrada (con 24 h de tolerancia por zonas horarias/DST), el período de gracia y la
  // expiración del token dejan de ser confiables → exigir verificación online.
  // Sin esto, retrasar la fecha de Windows congelaría la gracia offline indefinidamente.
  const CLOCK_ROLLBACK_TOLERANCE_MS = 24 * 60 * 60 * 1000;
  if (Number.isFinite(last) && Date.now() < last - CLOCK_ROLLBACK_TOLERANCE_MS) {
    return {
      ok: false, state: 'clock_rollback',
      reason: 'La fecha del sistema es anterior a la última verificación de licencia. Corrige la fecha/hora del equipo o conéctate a internet para revalidar.',
      info: publicInfo(local)
    };
  }
