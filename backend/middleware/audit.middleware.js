 ...
 *
 * @param {*} [_db] Parámetro legacy ignorado (antes recibía el db/t de pg-promise);
 *                  el wrapper SQLite es singleton. Se mantiene por compatibilidad de firma.
 * @param {object} opts
 ...
 */
async function registrarAuditoria(_db, opts) {
  const {
    usuario_id,
    accion,
    tabla_afectada = null,
  ...
  if (uid != null) {
    const existe = getOne(`SELECT 1 AS ok FROM usuarios WHERE id = ? LIMIT 1`, [uid]);
    if (!existe) {
      logger.warn(
