 ...
 * SÍNCRONA: usable dentro de transaction() de better-sqlite3; los llamadores
 * con `await` siguen funcionando (await de valor no-promesa).
 * @param {*} _tOrDb Parámetro legacy ignorado (wrapper SQLite singleton).
 * @param {number} ventaId
 * @returns {Map<number, number>}
 */
function loadDevolucionesPreviasMap(_tOrDb, ventaId) {
  const rows = getAll(
    `SELECT lineas FROM devoluciones WHERE venta_id = ? AND estado != 'anulada'`,
    [ventaId]
  );
  const map = new Map();
...
/** SÍNCRONA: usable dentro de transaction() (await de valor no-promesa sigue OK). */
function resolverTasaBcvVenta(_tOrDb, venta) {
