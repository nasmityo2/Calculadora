...
async function listCategorias(req, res) {
  const rows = getAll(
    `SELECT id, nombre, descripcion, color_hex, icono
     FROM categorias WHERE activa = 1 ORDER BY nombre`
  );
  res.json(rows);
}
...
async function createCategoria(req, res) {
  ...
  const existing = getOne(
    `SELECT id FROM categorias WHERE LOWER(nombre) = LOWER(?) AND activa = 1`,
    [nombre]
  );
  ...
  const row = getOne(`SELECT id, nombre FROM categorias WHERE id = ?`, [r.lastInsertRowid]);
...
async function ajusteMasivo(req, res) {
  ...
  let whereClause = 'activo = 1 AND COALESCE(costo_usd, 0) > 0'
  ...
  let updateExpr;
  if (tipo === 'nuevo_fijo')   updateExpr = '?';
  else if (tipo === 'incremento') updateExpr = 'margen_ganancia_pct + ?';
  else                            updateExpr = 'MAX(0, margen_ganancia_pct - ?)';
  ...
  const result = run(
    `UPDATE productos
     SET margen_ganancia_pct = ${updateExpr},
         actualizado_en = ?
     WHERE ${whereClause}`,
    [params[0], new Date().toISOString(), ...params.slice(1)]
  );
  ...
  res.json({
    ok: true,
    productos_actualizados: result.changes,
    message: `Se actualizaron ${result.changes} productos correctamente`
  });
}
  ...
  transaction(() => {
    run(
      `UPDATE productos
       ...
       WHERE id = ?`,
      [cantidadEntera, new Date().toISOString(), producto_id]
    );
...
async function movimientos(req, res) {
  ...
  const existe = getOne(`SELECT id FROM productos WHERE id = ?`, [productoId]);
