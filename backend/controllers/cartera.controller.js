...
/** Días transcurridos desde el vencimiento (TEXT ISO → julianday). */
const DIAS_VENCIDOS_SQL = `CAST(julianday(date('now')) - julianday(date(cc.fecha_vencimiento)) AS INTEGER)`;
...
async function resumen(req, res) {
  const totales = getOne(`
    SELECT
      ...
      COALESCE(SUM((${SALDO_BCV_SQL})),0)                   AS total_deuda_bcv,
      COALESCE(SUM(cc.saldo_pendiente_usd),0)               AS total_deuda_usd,
      SUM(CASE WHEN cc.estado='vencida' THEN 1 ELSE 0 END)  AS cuentas_vencidas,
      COALESCE(SUM(CASE WHEN cc.estado='vencida' THEN (${SALDO_BCV_SQL}) ELSE 0 END),0)
                                                            AS deuda_vencida_bcv,
      COALESCE(SUM(CASE WHEN cc.estado='vencida' THEN cc.saldo_pendiente_usd ELSE 0 END),0)
  ...
  const buckets = getAll(`
    SELECT
      CASE
        WHEN ${DIAS_VENCIDOS_SQL} <= 0     THEN 'corriente'
        WHEN ${DIAS_VENCIDOS_SQL} <= 30    THEN '1_30'
        WHEN ${DIAS_VENCIDOS_SQL} <= 60    THEN '31_60'
        WHEN ${DIAS_VENCIDOS_SQL} <= 90    THEN '61_90'
        ELSE '91_mas'
      END AS bucket,
      COUNT(*)                                AS cuentas,
      COALESCE(SUM((${SALDO_BCV_SQL})),0)     AS monto_bcv,
      COALESCE(SUM(cc.saldo_pendiente_usd),0) AS monto_usd
    FROM cuentas_cobrar cc
    LEFT JOIN ventas v ON v.id = cc.venta_id
    WHERE cc.estado IN ('pendiente','vencida')
  ...
  const alertas = getAll(`
    SELECT c.id, c.nombre, c.telefono,
           COALESCE(SUM((${SALDO_BCV_SQL})),0)     AS deuda_bcv,
           COALESCE(SUM(cc.saldo_pendiente_usd),0) AS deuda_usd,
...
async function listCuentas(req, res) {
  ...
  // Filter params primero, pagination params siempre al final.
  const filterParams = [];
  ...
  if (estado && ['pendiente','vencida','pagada'].includes(estado)) {
    ...
    conds.push(`cc.estado = ?`);
  }
  if (cliente_id) {
    filterParams.push(Number(cliente_id));
    conds.push(`cc.cliente_id = ?`);
  }
  ...
  const rows = getAll(`
    SELECT
      ...
      (${SALDO_BCV_SQL}) AS saldo_pendiente_bcv,
      cc.monto_usd_bcv   AS monto_original_bcv,
      cc.estado,
      ...
      CASE
        ...
        WHEN date(cc.fecha_vencimiento) < date('now') THEN ${DIAS_VENCIDOS_SQL}
        ELSE 0
      END AS dias_vencida
    FROM cuentas_cobrar cc
    JOIN clientes c ON c.id = cc.cliente_id
  ...
  const result = transaction(() => {
    const cuenta = getOne(
      `SELECT cc.*, c.nombre AS cliente_nombre,
       ...
       JOIN clientes c ON c.id = cc.cliente_id
       LEFT JOIN ventas v ON v.id = cc.venta_id
       WHERE cc.id = ? AND cc.estado IN ('pendiente','vencida')`,
      [cuentaId]
    );
    if (!cuenta) throw httpError(404, 'Cuenta no encontrada o ya pagada');

    const resuelto = resolverMontoAbono(req.body || {}, cuenta, null);
    ...
    run(
      `UPDATE cuentas_cobrar
          SET saldo_pendiente_usd = ?,
              ...
              actualizado_en      = ?
        WHERE id = ?`,
      [nuevoSaldo.toFixed(4), nuevoSaldo.toFixed(4), ahoraIso, cuentaId]
    );
    ...
    run(
      `UPDATE clientes
          SET saldo_deuda_usd = MAX(0, COALESCE(saldo_deuda_usd,0) - ?),
              actualizado_en  = ?
        WHERE id = ?`,
      [aplicar.toFixed(4), ahoraIso, cuenta.cliente_id]
    );
...
async function estadoCuentaPdf(req, res) {
  ...
  const cliente = getOne(`SELECT * FROM clientes WHERE id = ?`, [clienteId]);
