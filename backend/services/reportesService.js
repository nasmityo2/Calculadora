...
/** Límites UTC (ISO) del día operativo Caracas (UTC−4 fijo) con offset en días. */
...
class ReportesService {
  ...
  static async ventasDia(_db, fecha) {
    const trimmed = fecha != null ? String(fecha).trim() : '';
    let fechaCond;
    const params = [];
    if (!trimmed) {
      const hoy = limitesDiaCaracas(0);
      fechaCond = `v.fecha_venta >= ? AND v.fecha_venta < ?`;
      params.push(hoy.inicio, hoy.fin);
    } else {
      ...
      fechaCond = `v.fecha_venta >= ? AND v.fecha_venta < date(?, '+1 day')`;
      params.push(ymd, ymd);
    }
    ...
    // items: TEXT JSON (json_group_array) → array (PG devolvía JSONB)
    return rows.map((r) => {
      let items = r.items;
      if (typeof items === 'string') {
        try { items = JSON.parse(items); } catch (_e) { items = null; }
      }
      return { ...r, items: items && items.length ? items : null };
    });
  }

  static async ventasPeriodo(_db, diasAtras) {
    const dias = ReportesService._diasVentana(diasAtras, 7);
    const desde = new Date(Date.now() - dias * 86400000).toISOString();
    const rows = getAll(`
      SELECT DATE(fecha_venta, '-4 hours') AS fecha,
             COUNT(*) AS num_ventas,
             ...
             COALESCE(AVG(COALESCE(total_ref_usd_bcv, total_usd)), 0) AS ticket_promedio_bcv
      FROM ventas
      WHERE estado = 'completada'
        AND fecha_venta >= ?
      GROUP BY DATE(fecha_venta, '-4 hours')
      ORDER BY fecha
    `, [desde]);
    return rows;
  }

  static async topProductos(_db, limite, diasAtras) {
    ...
    const rows = getAll(`
      SELECT p.nombre, p.codigo_barras,
             ...
             COALESCE(SUM(dv.subtotal_usd - dv.costo_unitario_usd * dv.cantidad), 0) AS ganancia_usd,
             CASE WHEN SUM(dv.subtotal_usd) > 0
                  THEN ROUND(SUM(dv.subtotal_usd - dv.costo_unitario_usd * dv.cantidad)
                       / SUM(dv.subtotal_usd) * 100, 1)
                  ELSE 0
             END AS margen_pct
      FROM detalles_ventas dv
      JOIN productos p    ON p.id = dv.producto_id
      ...
      ORDER BY ingresos_usd DESC LIMIT ?
    `, [desde, lim]);
    return rows;
  }

  static async rentabilidadCategorias(_db, diasAtras) {
    const dias = ReportesService._diasVentana(diasAtras, 30);
    const desde = new Date(Date.now() - dias * 86400000).toISOString();
    const rows = getAll(`
  ...
  static async sugerenciaReposicion(_db) {
    ...
    const rows = getAll(`
      WITH velocidad AS (
        SELECT
          ...
          p.stock_actual,
          p.stock_minimo,
          p.costo_usd AS costo_aterrizaje_usd,
          p.margen_ganancia_pct,
          COALESCE(cat.nombre, 'Sin categoría') AS categoria,
      ...
      con_prioridad AS (
        SELECT *,
          ...
          CAST(CEIL(ventas_diarias_promedio * 30) AS INTEGER) AS cantidad_sugerida,
          CAST(CEIL(ventas_diarias_promedio * 30) AS INTEGER) * costo_aterrizaje_usd AS inversion_sugerida_usd,
          CASE
  ...
  static async deudasClientes(_db) {
    const saldoBcvExpr = `
      COALESCE(
    ...
    const rows = getAll(`
      SELECT
        ...
        COUNT(cc.id)                                              AS num_deudas,
        COALESCE(SUM((${saldoBcvExpr})), 0)                       AS deuda_total_bcv,
        COALESCE(SUM(cc.saldo_pendiente_usd), 0)                  AS deuda_total_usd,
  ...
  static async historialCierresCaja(_db, limite) {
    ...
    const rows = getAll(`
      SELECT sc.*,
             ...
             (
               SELECT COALESCE(SUM(total_usd), 0) FROM ventas v
               WHERE v.sesion_caja_id = sc.id AND v.estado = 'completada'
  ...
  static async ventasPorCajero(_db, diasAtras) {
    const dias = ReportesService._diasVentana(diasAtras, 30);
    const desde = new Date(Date.now() - dias * 86400000).toISOString();
    const rows = getAll(`
      SELECT u.nombre_completo AS cajero,
             ...
             COALESCE(AVG(v.total_usd), 0)           AS ticket_promedio,
             COALESCE(AVG(COALESCE(v.total_ref_usd_bcv, v.total_usd)), 0) AS ticket_promedio_bcv,
             COALESCE(SUM(
  ...
  static async ventasRango(_db, desde, hasta) {
    ...
    const rows = getAll(`
      ...
      LEFT JOIN clientes c ON c.id = v.cliente_id
      WHERE v.estado = 'completada'
        AND v.fecha_venta >= ? AND v.fecha_venta < date(?, '+1 day')
      ORDER BY v.fecha_venta DESC
    `, [desdeStr, hastaStr]);
    return rows;
  }
  ...
  static async ventasRangoResumen(_db, desde, hasta) {
    const { desde: desdeStr, hasta: hastaStr } = ReportesService._rangoFechas(desde, hasta, 30);
    const rows = getAll(`
      SELECT DATE(fecha_venta, '-4 hours') AS fecha,
             COUNT(*)                         AS num_ventas,
      ...
      FROM ventas
      WHERE estado = 'completada'
        AND fecha_venta >= ? AND fecha_venta < date(?, '+1 day')
      GROUP BY DATE(fecha_venta, '-4 hours')
      ORDER BY fecha
    `, [desdeStr, hastaStr]);
    return rows;
  ...
  static async _inventarioValorizado(_db) {
   ...
   */
  static async liquidacionesCasheaPorDeposito(_db, desde, hasta) {
    const { desde: d0, hasta: d1 } = ReportesService._rangoFechas(desde, hasta, 30);
    // resolverTasasOperativas: defensa en lectura (unifica tasa_usd = tasa_bcv en solo_bcv).
    const tasasActuales = await PreciosService.resolverTasasOperativas();
    const fallbackBcv = tasasActuales.bcv;
    const fallbackUsd = tasasActuales.tasa_usd;

    const detalleRaw = getAll(`
      SELECT cl.id,
             cl.fecha_liquidacion,
             DATE(cl.fecha_liquidacion) AS fecha_deposito,
             cl.semana_inicio,
             cl.semana_fin,
             cl.total_bruto_usd,
             cl.total_comisiones_usd,
             cl.total_neto_usd,
             cl.cantidad_ventas,
      ...
      WHERE cl.fecha_liquidacion IS NOT NULL
        AND cl.fecha_liquidacion >= ?
        AND cl.fecha_liquidacion < date(?, '+1 day')
      ORDER BY cl.fecha_liquidacion DESC, cl.id DESC
    `, [d0, d1]);

    const batchIds = detalleRaw.map((r) => r.id);
    const ventasPorBatchRows = batchIds.length
      ? getAll(`
          ...
          WHERE vc.liq_batch_id IN (${batchIds.map(() => '?').join(', ')})
        `, batchIds)
      : [];
    const historialRows = getAll(`
      SELECT fecha, tasa_bcv, tasa_usd
