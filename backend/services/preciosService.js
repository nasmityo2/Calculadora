...
class PreciosService {
  ...
  static async leerFeriadosBcvVe(_dbOrT) {
    const row = getOne(`SELECT valor FROM configuracion WHERE clave = 'tasa_bcv_feriados_ve'`);
   ...
   */
  static async leerTasaBcvVigenteLegal(_dbOrT, instant = new Date()) {
    const feriados = await PreciosService.leerFeriadosBcvVe(_dbOrT);
    const hoy = bcvVigencia.ymdCaracas(instant);
    const ref = bcvVigencia.diaHabilReferenciaTransaccion(hoy, feriados);

    const hist = getOne(`SELECT tasa_bcv FROM historial_tasas WHERE fecha = ?`, [ref]);
    if (hist && hist.tasa_bcv != null) {
      const bcv = PreciosService.redondearTasa4(hist.tasa_bcv);
      if (bcv > 0 && !Number.isNaN(bcv)) return bcv;
    }

    const rows = getAll(`SELECT clave, valor FROM configuracion WHERE clave = 'tasa_bcv'`);
   ...
   */
  static async leerImpuestoIvaPorcentaje(_dbOrT) {
    const row = getOne(`SELECT valor FROM configuracion WHERE clave = 'impuesto_iva'`);
   ...
   * SÍNCRONA: se usa dentro de transaction() de better-sqlite3 (prohibido await dentro).
   * @returns {{ tasa_bcv: number|null, tasa_usd: number|null }}
   */
  static leerTasasPreviasConfig(_t) {
    const rows = getAll(
      `SELECT clave, valor FROM configuracion
       WHERE clave IN ('tasa_bcv', 'tasa_usd')`
    );
  ...
  static async actualizarTasas(db, nueva_tasa_bcv, nueva_tasa_usd, usuario_id, ip_address = null) {
    ...
    const modo = await ModoMonedaService.leerModo(db);
    return PreciosService.actualizarTasasSync(nueva_tasa_bcv, nueva_tasa_usd, usuario_id, ip_address, modo);
  }

  /**
   * Versión SÍNCRONA de actualizarTasas para usar dentro de transaction() de
   * better-sqlite3 (las transacciones anidadas usan savepoints automáticamente).
   * El modo monetario debe venir ya leído (sin awaits dentro de la transacción).
   */
  static actualizarTasasSync(nueva_tasa_bcv, nueva_tasa_usd, usuario_id, ip_address, modo) {
    ...
    const fechaCaracas = bcvVigencia.ymdCaracas();
    ...
    transaction(() => {
      ...
      run(
        `INSERT INTO configuracion (clave, valor, categoria)
         VALUES ('tasa_bcv', ?, 'moneda')
         ON CONFLICT (clave) DO UPDATE
         SET valor = excluded.valor, actualizado_en = ?`,
        [textoBcv, ahoraIso]
      );
      run(
        `INSERT INTO configuracion (clave, valor, categoria)
      ...
      // Lógica del trigger PG registrar_historial_tasa (035): registro diario de tasas.
      run(
        `INSERT INTO historial_tasas (fecha, tasa_bcv, tasa_usd, registrado_por)
         VALUES (?, ?, ?, ?)
         ON CONFLICT (fecha) DO UPDATE SET
           tasa_bcv = excluded.tasa_bcv,
           tasa_usd = excluded.tasa_usd`,
        [fechaCaracas, bcv_4d, usd_4d, usuario_id || null]
      );

      // registrarAuditoria no contiene awaits internos: su cuerpo corre sincrónicamente
      // dentro de esta transacción (y nunca lanza).
  ...
  static async actualizarTasaBcvAutomatica(db, nueva_tasa_bcv, meta = {}) {
    ...
    // Lecturas previas FUERA de la transacción (transaction() es síncrona; sin await dentro).
    const modo = await ModoMonedaService.leerModo();
    const esSoloBcv = ModoMonedaService.esSoloBcv(modo);
   ...
   */
  static async resolverDescuentoCobroDivisaConfig(_dbOrT) {
    const rows = getAll(
      `SELECT clave, valor FROM configuracion
