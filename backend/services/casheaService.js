...
async function obtenerConfigFresh(_conn) {
  const row = getOne('SELECT * FROM cashea_config ORDER BY id ASC LIMIT 1');
  if (!row) {
    throw new Error('casheaService.obtenerConfigFresh: no existe fila de cashea_config');
  }
  return row;
}
 ...
 */
/**
 * SÍNCRONA (better-sqlite3): se invoca dentro del transaction() de ventas.create().
 * Mantiene nombre/posición de parámetros; `_db_t` legacy se ignora.
 */
function registrarPagoCashea(ventaId, desglose, nivelCliente, _db_t) {
...
async function obtenerResumenPendiente(fechaDesde, fechaHasta) {
  ...
  const rows = getAll(
    `SELECT vc.*,
     ...
     ${usarFiltroFecha ? `AND v.fecha_venta >= ? AND v.fecha_venta < date(?, '+1 day')` : ''}
...
async function procesarLiquidacion(payload) {
  ...
  transaction(() => {
    /* Bug-30: collect candidate IDs first, then use those exact IDs in both the
       totals calculation AND the UPDATE. En SQLite transaction() serializa las
       escrituras (sin FOR UPDATE), por lo que los dos conjuntos son idénticos. */
    ...
    const rBatch = run(
      ...
      [d0, d1, ahoraIso, totalBruto, totalCom, totalNeto, n, refBan || null, notasOpt || null]
    );
    batchRow = getOne(`SELECT * FROM cashea_liquidaciones WHERE id = ?`, [rBatch.lastInsertRowid]);
    ...
    if (candidateIds.length > 0) {
      ...
      run(
        ...
        [ahoraIso, batchRow.id, ...candidateIds]
      );
    }
...
async function listarLiquidaciones(page, limit) {
  ...
  const offset = (p - 1) * l;
  const totalRow = getOne(`SELECT COUNT(*) AS c FROM cashea_liquidaciones`);
  const rows = getAll(
    `SELECT * FROM cashea_liquidaciones ORDER BY created_at DESC LIMIT ? OFFSET ?`,
    [l, offset]
  );
...
async function detalleLiquidacion(id) {
  ...
  const batch = getOne(`SELECT * FROM cashea_liquidaciones WHERE id = ?`, [bid]);
...
async function sincronizarComisionesConfigDesdeTarifas(_conn) {
  const cfg = await obtenerConfigFresh();
  const linea = normalizarLineaComercial(cfg && cfg.linea_comercial);
