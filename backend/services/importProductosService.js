...
const { getDB, getOne, getAll, run } = require('../config/db');
...
function generarCodigoInternoUnico(used) {
  for (let i = 0; i < 25; i++) {
    const part = crypto.randomBytes(4).toString('hex').toUpperCase();
    const sku = `NC-${part}`;
    if (used.has(sku)) continue;
    const exists = getOne(
      `SELECT 1 AS ok FROM productos WHERE codigo_interno = ? LIMIT 1`, [sku]
    );
 ...
 * @param {*} _db Parámetro legacy ignorado (el wrapper SQLite es singleton).
 * @returns {{ importados, omitidos, total, filas }}
 */
async function importarProductosDesdeExcel(_db, fileBuffer) {
  let ExcelJS;
  ...
  const categorias = getAll(`SELECT id, LOWER(TRIM(nombre)) AS nombre FROM categorias`);
  const proveedores = getAll(`SELECT id, LOWER(TRIM(nombre)) AS nombre FROM proveedores`);
  ...
  // FASE A (async, parse Excel): construir candidatos en memoria.
  // FASE B (sync): transacción raíz con savepoint anidado por fila — un error
  // por fila (duplicado, FK inválida) revierte SOLO esa fila, igual que el
  // esquema pg-promise tx + savepoints original.
  const candidatos = [];
  ...
  for (let rowNum = headerRowNum + 1; rowNum <= ws.rowCount; rowNum++) {
    ...
    // Booleanos → INTEGER 0/1 (SQLite)
    const aplIva = parseBool(c.aplica_iva);
    payload.aplica_iva = (aplIva !== null ? aplIva : false) ? 1 : 0;

    const activo = parseBool(c.activo);
    payload.activo = (activo !== null ? activo : true) ? 1 : 0;

    candidatos.push({
      rowNum,
      nombre,
      payload,
      catNombreRaw: parseStr(c.cat_nombre),
      provNombreRaw: parseStr(c.prov_nombre)
    });
  }

  // ── FASE B: inserción en transacción raíz con savepoints por fila ────────
  const database = getDB();

  const insertarFila = database.transaction((cand) => {
    const { payload } = cand;

    // Categoría: buscar o crear (dentro del savepoint de la fila)
    if (cand.catNombreRaw) {
      const catKey = cand.catNombreRaw.toLowerCase().trim();
      if (catMap[catKey] !== undefined) {
        payload.categoria_id = catMap[catKey];
      } else {
        const existing = getOne(
          `SELECT id FROM categorias WHERE LOWER(TRIM(nombre)) = ? LIMIT 1`, [catKey]
        );
        if (existing) {
          catMap[catKey] = existing.id;
          payload.categoria_id = existing.id;
        } else {
          const rCat = run(`INSERT INTO categorias (nombre) VALUES (?)`, [cand.catNombreRaw.trim()]);
          catMap[catKey] = rCat.lastInsertRowid;
          payload.categoria_id = rCat.lastInsertRowid;
        }
      }
    }

    // Proveedor: solo buscar (no se crea automáticamente)
    if (cand.provNombreRaw) {
      const provKey = cand.provNombreRaw.toLowerCase().trim();
    ...
    if (!payload.codigo_interno) {
      payload.codigo_interno = generarCodigoInternoUnico(usedInternos);
    }
    ...
    const r = run(
      `INSERT INTO productos (${cols.join(', ')}) VALUES (${phs.join(', ')})`,
      vals
    );
    const rowDb = getOne(
      `SELECT id, nombre, codigo_interno FROM productos WHERE id = ?`,
      [r.lastInsertRowid]
    );
    // INV-04: el movimiento de stock inicial dentro del mismo savepoint de la fila.
  ...
  const importarTodo = database.transaction(() => {
    for (const cand of candidatos) {
      try {
        // transaction anidada = SAVEPOINT: si falla, solo se revierte esta fila.
        const inserted = insertarFila(cand);
        importados += 1;
        filas.push({
          fila: cand.rowNum,
        ...
        if (e.code === 'SQLITE_CONSTRAINT_UNIQUE') {
          if (e.message && e.message.includes('codigo_barras')) {
            razon = `Código de barras duplicado: "${cand.payload.codigo_barras || ''}"`;
          } else if (e.message && e.message.includes('codigo_interno')) {
            razon = `Código interno duplicado: "${cand.payload.codigo_interno || ''}"`;
          } else {
        ...
        filas.push({ fila: cand.rowNum, nombre: cand.nombre, estado: 'error', razon });
        omitidos += 1;
      }
    }
  });
