...
async function verificarTiempoExterno() {
  ...
  for (const url of ENDPOINTS) {
    try {
      const res = await getJson(url, { timeoutMs: TIMEOUT_MS });
      if (!res.ok) continue;
      const data = res.body;
...
// ── Bitácora local de verificaciones (migr. 043) ──────────────────────────

/** Clave enmascarada para bitácora: NUNCA la clave completa (regla anti-leak). */
function maskLicenseKey(clave) {
  const s = String(clave || '').trim();
  if (!s) return null;
  return s.length <= 12 ? s.slice(0, 4) + '…' : s.slice(0, 12) + '…';
}

/** Prefijo de hash del HWID (12 hex) — jamás el HWID real. */
function hwidPrefix(hwid) {
  const s = String(hwid || '').trim();
  if (!s) return null;
  return sha256hex(s.toUpperCase()).slice(0, 12);
}

/**
 * Inserta una fila en licencia_verificaciones. Best-effort: NUNCA lanza
 * (la bitácora no puede tumbar el flujo de licencias).
 * @param {{ evento:string, resultado:string, motivo?:string|null,
 *           tipo_licencia?:string|null, clave?:string|null, hwid?:string|null,
 *           origen?:string }} ev
 */
function registrarVerificacionLicencia(ev) {
  try {
    run(
      `INSERT INTO licencia_verificaciones (
         verificado_en, evento, resultado, motivo,
         tipo_licencia, license_key_masked, hwid_prefix, origen
       ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        new Date().toISOString(),
        String(ev.evento || 'verify'),
        String(ev.resultado || 'desconocido'),
        ev.motivo != null ? String(ev.motivo).slice(0, 300) : null,
        ev.tipo_licencia != null ? String(ev.tipo_licencia) : null,
        maskLicenseKey(ev.clave),
        hwidPrefix(ev.hwid),
        ev.origen ? String(ev.origen) : 'backend'
      ]
    );
  } catch (err) {
    logger.warn('[licencia] bitácora licencia_verificaciones no disponible', {
      error: err.message
    });
  }
}
 ...
 */
async function obtenerEstadoLicencia(_db, hwid) {
  ...
  const claveRow = getOne(`SELECT valor FROM configuracion WHERE clave = 'licencia_clave' LIMIT 1`);
  ...
  // Bitácora: solo se registran rechazos (clave presente pero inválida para este equipo).
  // Los chequeos exitosos de rutina no se loguean para no inflar la tabla en cada arranque.
  registrarVerificacionLicencia({
    evento: 'startup',
    resultado: 'rejected',
    motivo: lastMotivo,
    clave: claveRow.valor,
    hwid: primary,
    origen: 'backend'
  });
 ...
 */
async function activarLicencia(_db, clave, hwid) {
  ...
  transaction(() => {
    ...
    upsert('licencia_clave', clave, 'Clave de activación (Ed25519)');
    upsert('licencia_hwid', hwid, 'Hardware ID registrado');
    upsert('licencia_empresa', result.info.empresa, 'Empresa de la licencia');
    upsert('licencia_edition', result.info.edition, 'Edición de la licencia');
    upsert('licencia_expira', result.info.expira, 'Fecha de expiración');
    upsert(
  ...
  registrarVerificacionLicencia({
    ...
    resultado: 'ok',
    motivo: null,
    tipo_licencia: result.info.esTrial ? 'trial' : (result.info.expira === 'Perpetua' ? 'permanent' : 'subscription'),
    clave,
    hwid,
    origen: 'backend'
  });
