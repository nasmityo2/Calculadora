...
class ExcelService {
  ...
  static async exportarControlPrecios(db) {
    ...
    const productos = getAll(`
      SELECT p.nombre, p.codigo_interno, p.codigo_barras,
             p.costo_usd,
             p.margen_ganancia_pct                        AS ganancia_pct,
             p.precio_manual_usd,
             p.stock_actual,
  ...
  static async exportarReporteVentas(db, diasAtras) {
    ...
    const rows = getAll(`
      SELECT v.numero_venta, v.fecha_venta,
             v.total_usd,
             COALESCE(v.total_ref_usd_bcv, v.total_usd) AS total_ref_usd_bcv,
             v.total_bs,
  ...
  static async exportarLibroVentas(db, mesAnio) {
    ...
    const ventas = getAll(`
      SELECT
        ...
        v.subtotal_usd                          AS base_imponible_usd,
        v.iva_monto_usd                         AS iva_usd,
        v.total_usd                             AS total_usd,
        v.total_bs                              AS total_bs,
        v.iva_porcentaje                        AS iva_pct,
        v.metodo_pago
      FROM ventas v
      LEFT JOIN clientes c ON c.id = v.cliente_id
  ...
  static async exportarLibroCompras(db, mesAnio) {
    ...
    const empresaRows = getAll(
      `SELECT clave, valor FROM configuracion WHERE clave IN ('nombre_empresa','rif_empresa')`
    );
    const empresa = {};
    empresaRows.forEach((r) => { empresa[r.clave] = r.valor; });
    // SQLite no soporta LATERAL: las bases gravada/exenta y el conteo de líneas
    // van como subconsultas escalares correlacionadas en el SELECT.
    const comprasRows = getAll(`
      SELECT
        c.numero_compra,
        DATE(c.fecha_compra)              AS fecha,
        COALESCE(p.nombre, 'Proveedor')   AS proveedor,
        COALESCE(p.rif, 'V-00000000')    AS rif_proveedor,
        c.total_usd                       AS total_mercancia_usd,
        COALESCE((
          SELECT SUM(CASE WHEN COALESCE(pr.aplica_iva, 1) = 1 THEN dc.subtotal_usd ELSE 0 END)
          FROM detalles_compras dc
          JOIN productos pr ON pr.id = dc.producto_id
          WHERE dc.compra_id = c.id
        ), 0) AS base_gravada_usd,
        COALESCE((
          SELECT SUM(CASE WHEN pr.aplica_iva = 0 THEN dc.subtotal_usd ELSE 0 END)
          FROM detalles_compras dc
          JOIN productos pr ON pr.id = dc.producto_id
          WHERE dc.compra_id = c.id
        ), 0) AS base_exenta_usd,
        COALESCE((
          SELECT COUNT(*) FROM detalles_compras dc2 WHERE dc2.compra_id = c.id
        ), 0) AS num_lineas,
        c.notas
      FROM compras c
    ...
    `, [desde, hasta]);
    const ivaPct = await PreciosService.leerImpuestoIvaPorcentaje();
