...
const bcvVigenciaPdf = require('../utils/bcvVigenciaVe');
const {
  formatBolivares,
  formatUsdRef,
  formatTasaBcv,
...
/** Límites UTC (ISO) del día operativo Caracas (UTC−4 fijo) para cierres térmicos. */
function limitesDiaCaracasPdf(offsetDias = 0) {
  const ymd = bcvVigenciaPdf.ymdCaracas();
...
async function loadEmpresa(_db) {
  const rows = getAll(
...
async function fetchVentaPdfContext(_db, ventaId) {
  const venta = getOne(
...
async function fetchFacturaHtmlContext(_db, ventaId) {
...
/** PDF A4 — Cotización formal */
function generateCotizacionPdfBuffer(ctx) {
  const { empresa, cotizacion, lineas, tasa } = ctx;
  const doc = new jsPDF({ unit: 'mm', format: 'a4', orientation: 'portrait' });
  const mx = 14;
  const pageRight = 196;
  const contentW = pageRight - 2 * mx;
  ...
  doc.setFontSize(15);
  doc.setTextColor(...NEXUS_PRINT.navy);
  doc.text(String(empresa.nombre || '').toUpperCase(), mx, y);
  y += 6;

  doc.setFont('helvetica', 'normal');
  doc.setFontSize(9);
  doc.setTextColor(...NEXUS_PRINT.text);
  if (empresa.rif) { doc.text(`RIF: ${empresa.rif}`, mx, y); y += 4.5; }
  if (empresa.direccion) {
    const ad = doc.splitTextToSize(empresa.direccion, 110);
    doc.text(ad, mx, y); y += ad.length * 4.5;
  }
  if (empresa.telefono) { doc.text(`Tel: ${empresa.telefono}`, mx, y); y += 4.5; }
  if (empresa.email) { doc.text(empresa.email, mx, y); y += 4.5; }

  // Badge COTIZACIÓN (superior derecho)
  const badgeW = 56;
  const badgeH = 9;
  const badgeX = pageRight - mx - badgeW;
  doc.setFillColor(...NEXUS_PRINT.amber);
  doc.rect(badgeX, 12, badgeW, badgeH, 'F');
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(11);
  doc.setTextColor(...NEXUS_PRINT.textOnAccent);
  doc.text('COTIZACIÓN', badgeX + badgeW / 2, 18, { align: 'center' });

  doc.setFont('courier', 'bold');
  doc.setFontSize(10);
  doc.setTextColor(...NEXUS_PRINT.navy);
  doc.text(cotizacion.numero || '', pageRight - mx, 28, { align: 'right' });

  doc.setFont('helvetica', 'normal');
  doc.setFontSize(9);
  doc.setTextColor(...NEXUS_PRINT.text);
  const fmtD = (d) => { if (!d) return '—'; const dt = d instanceof Date ? d : new Date(d); return dt.toLocaleDateString('es-VE', { day: '2-digit', month: '2-digit', year: 'numeric' }); };
  doc.text(`Emisión: ${fmtD(cotizacion.fecha_emision)}`, pageRight - mx, 34, { align: 'right' });
  doc.text(`Vence: ${fmtD(cotizacion.fecha_vencimiento)}`, pageRight - mx, 39, { align: 'right' });
  doc.text(`Tasa BCV: ${formatTasaBcv(tasa)}`, pageRight - mx, 44, { align: 'right' });

  const estadoColors = {
    borrador:  NEXUS_PRINT.textMuted,
    enviada:   [59, 130, 246],
    aceptada:  [34, 197, 94],
    rechazada: [239, 68, 68],
    vencida:   [245, 158, 11],
  ...
  const estadoC = estadoColors[cotizacion.estado] || NEXUS_PRINT.textMuted;
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(8);
  doc.setTextColor(...estadoC);
  doc.text(String(cotizacion.estado || '').toUpperCase(), pageRight - mx, 49, { align: 'right' });

  y = Math.max(y, 56);
  nexusPdfAccentLine(doc, y, mx, pageRight - mx, 0.55);
  y += 8;
  ...
  doc.setFont('helvetica', 'normal');
  doc.setTextColor(...NEXUS_PRINT.text);
  doc.text(cotizacion.cliente_nombre || 'Sin cliente asignado', mx, y);
  y += 4.5;
  if (cotizacion.cliente_doc) { doc.text(`Doc: ${cotizacion.cliente_doc}`, mx, y); y += 4.5; }
  if (cotizacion.cliente_telefono) { doc.text(`Tel: ${cotizacion.cliente_telefono}`, mx, y); y += 4.5; }

  // Elaborado por
  if (cotizacion.usuario_nombre) {
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(9);
    doc.setTextColor(...NEXUS_PRINT.navy);
    doc.text('ELABORADO POR', mx + 100, y - (cotizacion.cliente_telefono ? 13.5 : 9));
    doc.setFont('helvetica', 'normal');
    doc.setTextColor(...NEXUS_PRINT.text);
    doc.text(cotizacion.usuario_nombre, mx + 100, y - (cotizacion.cliente_telefono ? 9 : 4.5));
  }
  ...
  const body = lineas.map((l) => [
    ...
    formatUsdRef(l.precio_unitario_usd),
    formatBolivares(l.precio_unitario_bs),
    formatBolivares(l.subtotal_bs),
    formatUsdRef(l.subtotal_usd)
  ]);
  ...
  autoTable(doc, {
    startY: y,
    head: [['Descripción', 'Cant.', 'P.U. (USD)', 'P.U. (Bs.)', 'Subt. (Bs.)', 'Subt. (USD)']],
    body,
    theme: 'striped',
    tableWidth: contentW,
    headStyles: {
    ...
    styles: { fontSize: 9, cellPadding: 1.8, textColor: NEXUS_PRINT.text },
    alternateRowStyles: { fillColor: NEXUS_PRINT.stripe },
    columnStyles: {
      0: { cellWidth: 60 },
      1: { halign: 'right', cellWidth: 14, font: 'courier' },
      2: { halign: 'right', cellWidth: 24, font: 'courier' },
      3: { halign: 'right', cellWidth: 26, font: 'courier' },
  ...
  const descPct = Number(cotizacion.descuento_porcentaje) || 0;
  const ivaPct  = Number(cotizacion.iva_porcentaje) || 0;

  const subtotalBs  = usdRefToBs(cotizacion.subtotal_usd, tasaNum);
  const descMontoUsd = Number(cotizacion.descuento_monto_usd) || 0;
  const descMontoBs  = usdRefToBs(descMontoUsd, tasaNum);
  const ivaMontoUsd  = Number(cotizacion.iva_monto_usd) || 0;
  const ivaMontoBs   = usdRefToBs(ivaMontoUsd, tasaNum);

  const totalsRows = [
    ['Subtotal (Bs.)', formatBolivares(subtotalBs)],
    ['Subtotal ref. USD', formatUsdRef(cotizacion.subtotal_usd), 'ref']
  ];
  if (descPct > 0) {
    totalsRows.push([`Descuento (${fmtMoney(descPct, 1)}%)`, `−${formatBolivares(descMontoBs)}`]);
    totalsRows.push([`Descuento ref. USD`, `−${formatUsdRef(descMontoUsd)}`, 'ref']);
  }
  if (ivaPct > 0) {
    totalsRows.push([`IVA (${fmtMoney(ivaPct, 1)}%)`, formatBolivares(ivaMontoBs)]);
    totalsRows.push([`IVA ref. USD`, formatUsdRef(ivaMontoUsd), 'ref']);
  }

  const yDespuesTotales = nexusPdfDrawTotalsBlock(
    doc, y, tx, tw,
    totalsRows,
    ['TOTAL BOLÍVARES', formatBolivares(Number(cotizacion.total_bs) || 0)],
    ['Total referencia USD', formatUsdRef(Number(cotizacion.total_usd) || 0)]
  );

  // Notas — posicionadas debajo del bloque de totales
  if (cotizacion.notas) {
    y = yDespuesTotales + 8;
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(9);
    ...
    doc.setFont('helvetica', 'normal');
    doc.setTextColor(...NEXUS_PRINT.text);
    const notaLines = doc.splitTextToSize(cotizacion.notas, contentW);
  ...
  // Pie de firma
  const sigY = 270;
  nexusPdfNavyLine(doc, sigY, mx, mx + 72);
  nexusPdfNavyLine(doc, sigY, pageRight - mx - 72, pageRight - mx);
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(8);
  doc.setTextColor(...NEXUS_PRINT.textMuted);
  doc.text('Autorizado por', mx + 36, sigY + 5, { align: 'center' });
  doc.text('Aceptado por el cliente', pageRight - mx - 36, sigY + 5, { align: 'center' });

  doc.setFontSize(7.5);
  const pie = `Esta cotización tiene validez hasta el ${typeof fmtD === 'function' ? fmtD(cotizacion.fecha_vencimiento) : '—'}. Precios expresados en USD (referencia BCV) y su equivalente en Bolívares a tasa ${formatTasaBcv(tasa)} Bs/USD.`;
  const pieLines = doc.splitTextToSize(pie, contentW);
  doc.text(pieLines, mx, sigY + 12);
...
class PdfService {
   ...
   */
  static async generarCierreTermicoResumenDia(_db, empresa) {
    const hoyLim = limitesDiaCaracasPdf(0);
    const resumenDia = getOne(
      ...
      [hoyLim.inicio, hoyLim.fin]
    );

    /* Bug-29: aggregate over JSON pagos array (same logic as resumenCierre) instead of
       the scalar metodo_pago header column which only works for single-method sales. */
    const montosEsperados = getOne(
      `SELECT
         COALESCE(SUM(CASE WHEN json_extract(p.value, '$.metodo') = 'efectivo_usd' THEN CAST(json_extract(p.value, '$.monto') AS NUMERIC) ELSE 0 END), 0) AS efectivo_usd,
         COALESCE(SUM(CASE WHEN json_extract(p.value, '$.metodo') = 'efectivo_bs' THEN CAST(json_extract(p.value, '$.monto') AS NUMERIC) ELSE 0 END), 0) AS efectivo_bs,
         COALESCE(SUM(CASE WHEN json_extract(p.value, '$.metodo') = 'zelle' THEN CAST(json_extract(p.value, '$.monto') AS NUMERIC) ELSE 0 END), 0) AS zelle_usd,
         COALESCE(SUM(CASE WHEN json_extract(p.value, '$.metodo') = 'transferencia_bs' THEN CAST(json_extract(p.value, '$.monto') AS NUMERIC) ELSE 0 END), 0) AS transferencia_bs,
         COALESCE(SUM(CASE WHEN json_extract(p.value, '$.metodo') = 'pago_movil' THEN CAST(json_extract(p.value, '$.monto') AS NUMERIC) ELSE 0 END), 0) AS pago_movil_bs,
         COALESCE(SUM(CASE WHEN json_extract(p.value, '$.metodo') = 'punto' THEN CAST(json_extract(p.value, '$.monto') AS NUMERIC) ELSE 0 END), 0) AS punto_bs
       FROM ventas v,
            json_each(
    ...
    // en lugar del campo monto del JSON pagos que solo contiene la cuota inicial.
    const casheaResumenDia = getOne(
      `SELECT
         COALESCE(SUM(vc.total_venta_usd), 0)   AS cashea_total_facturado,
         COALESCE(SUM(vc.monto_inicial_usd), 0) AS cashea_inicial_cobrado
    ...
    const fechaTexto = `${fmtDate(hoyLim.ymd)} — Ventas del día (sin sesión de caja)`;
   ...
   */
  static async generarCierreTermico(_db, sesionIdParam) {
    const empresa = await loadEmpresa();
    let sesion = null;
    ...
    if (sesionIdParam && Number.isFinite(sesionIdParam) && sesionIdParam > 0) {
      sesion = getOne(`${sesionSelect} WHERE sc.id = ?`, [sesionIdParam]);
      if (!sesion) {
      ...
      const hoyLim = limitesDiaCaracasPdf(0);
      const hace14d = limitesDiaCaracasPdf(-14);
      sesion = getOne(
        `${sesionSelect}
      ...
      if (!sesion) {
        sesion = getOne(
          `${sesionSelect}
           WHERE sc.estado = 'cerrada' AND sc.fecha_apertura >= ? AND sc.fecha_apertura < ?
    ...
    if (!sesion) {
      return PdfService.generarCierreTermicoResumenDia(null, empresa);
    }
    ...
    /* Bug-29: aggregate over JSON pagos array so mixed-method sales are counted correctly. */
    const montosEsperados = getOne(
      `SELECT
    ...
    // Volumen Cashea correcto: usar ventas_cashea.total_venta_usd, no el monto del JSON pagos.
    const casheaResumenSesion = getOne(
      `SELECT
         COALESCE(SUM(vc.total_venta_usd), 0)   AS cashea_total_facturado,
         COALESCE(SUM(vc.monto_inicial_usd), 0) AS cashea_inicial_cobrado
       FROM ventas_cashea vc
  ...
  static generateCotizacionPdfBuffer(ctx) {
    return generateCotizacionPdfBuffer(ctx);
  }
}
