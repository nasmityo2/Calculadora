...
router.get('/resumen', asyncHandler(async (req, res) => {
  const resumen = await DashboardService.obtenerResumen(null, {
...
router.get('/ganancia-hoy', asyncHandler(async (req, res) => {
  const ganancia = await DashboardService.obtenerGananciaHoy(null);
  res.json(ganancia);
}));

// GET /api/dashboard/kpis
router.get('/kpis', asyncHandler(async (req, res) => {
  const kpis = await DashboardService.obtenerKpis(null);
  res.json(mapKpisResponse(kpis));
}));

// GET /api/dashboard/ventas-por-hora
router.get('/ventas-por-hora', asyncHandler(async (req, res) => {
  const data = await DashboardService.obtenerVentasPorHora(null);
  res.json(data);
}));
...
router.get('/ventas-30-dias', asyncHandler(async (req, res) => {
  ...
  res.json({
    fechas: rows.map((r) => new Date(`${r.fecha}T12:00:00-04:00`).toLocaleDateString('es-VE', { day: '2-digit', month: 'short' })),
...
router.get('/top-productos', asyncHandler(async (req, res) => {
  const data = await DashboardService.obtenerTopProductos(null, 5);
  res.json(data);
}));

// GET /api/dashboard/alertas-stock
router.get('/alertas-stock', asyncHandler(async (req, res) => {
  const data = await DashboardService.obtenerAlertasStock(null, 10);
  res.json(data);
}));
...
router.get('/distribucion-categorias', asyncHandler(async (req, res) => {
  ...
  const rows = getAll(`
    SELECT COALESCE(cat.nombre, 'Sin categoría') AS categoria,
           COALESCE(SUM(dv.subtotal_usd), 0) AS total_usd,
           COALESCE(SUM(
             dv.subtotal_usd
             * CASE
                 WHEN v.total_usd > 0
...
router.get('/ultimas-ventas', asyncHandler(async (req, res) => {
  const data = await DashboardService.obtenerUltimasVentas(null, 10);
  res.json(data);
}));

// GET /api/dashboard/alertas
router.get('/alertas', asyncHandler(async (req, res) => {
  const stockBajo = await DashboardService.obtenerAlertasStock(null, 50);
  const porVencer = await DashboardService.obtenerPorVencer(null, 20);
  const deudas = await DashboardService.obtenerDeudasVencidas(null, 50);
...
router.get('/resumen-ayer', asyncHandler(async (req, res) => {
  // Día operativo Caracas (UTC−4): límites calculados en JS, igual que dashboardService.
  const ymd = new Intl.DateTimeFormat('en-CA', {
    timeZone: 'America/Caracas', year: 'numeric', month: '2-digit', day: '2-digit'
  }).format(new Date());
  const hoyInicio = new Date(`${ymd}T00:00:00-04:00`);
  ...
  const r = getOne(`
    ...
    WHERE estado = 'completada'
      AND fecha_venta >= ?
      AND fecha_venta < ?
  `, [ayerInicio.toISOString(), hoyInicio.toISOString()]);
