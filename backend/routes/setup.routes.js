...
router.get('/estado', asyncHandler(async (_req, res) => {
  const estado = await setupAdminService.obtenerEstadoSetupAdmin(null);
  res.json(estado);
}));
...
router.post('/empresa-cashea-inicial', asyncHandler(async (req, res) => {
  try {
    const result = await setupAdminService.guardarEmpresaInicial(null, req.body || {});
...
router.post('/modo-moneda-inicial', asyncHandler(async (req, res) => {
  ...
  try {
    const result = await setupAdminService.guardarModoMonedaInicial(null, modo);
    res.status(200).json({ ok: true, ...result });
  } catch (e) {
...
router.post('/admin-inicial', asyncHandler(async (req, res) => {
  ...
  try {
    const user = await setupAdminService.crearAdminInicial(null, payload, ip);
    res.status(201).json({
      ok: true,
      user: {
        id: user.id,
