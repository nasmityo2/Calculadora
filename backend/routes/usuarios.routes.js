...
/** permisos_override viaja como TEXT JSON en SQLite → objeto para la API. */
...
router.post('/:id/cambiar-password', asyncHandler(async (req, res) => {
  ...
  const user = getOne(`SELECT * FROM usuarios WHERE id = ?`, [userId]);
 ...
 */
function fetchOverrides(userIds) {
  const map = new Map();
  if (!userIds || !userIds.length) return map;
  try {
    const placeholders = userIds.map(() => '?').join(', ');
    const rows = getAll(
      `SELECT id, COALESCE(permisos_override, '{}') AS permisos_override
       FROM usuarios WHERE id IN (${placeholders})`,
      userIds
    );
    rows.forEach((r) => map.set(r.id, parseJsonObj(r.permisos_override)));
...
router.get('/', asyncHandler(async (req, res) => {
  ...
  const overrides = fetchOverrides(rows.map((r) => r.id));
...
router.get('/roles', asyncHandler(async (req, res) => {
  const rows = getAll(`SELECT id, nombre FROM roles ORDER BY nombre`);
  res.json(rows);
}));
...
router.post('/roles', requirePermission('usuarios_all'), asyncHandler(async (req, res) => {
  ...
  const existing = getOne(`SELECT id FROM roles WHERE LOWER(nombre) = LOWER(?)`, [nombre]);
  ...
  const row = getOne(`SELECT id, nombre FROM roles WHERE id = ?`, [r.lastInsertRowid]);
...
router.get('/:id', asyncHandler(async (req, res) => {
  ...
  row.rol_permisos = parseJsonObj(row.rol_permisos);
  const overrides = fetchOverrides([userId]);
  res.json({ ...row, permisos_override: overrides.get(userId) || {} });
}));

function resolverRolId(body, fallbackActual = null) {
  ...
  const row = getOne(`SELECT id FROM roles WHERE LOWER(TRIM(nombre)) = ?`, [rolNombre]);
...
router.post('/', asyncHandler(async (req, res) => {
  ...
  const existing = getOne(
    `SELECT id FROM usuarios WHERE LOWER(username) = LOWER(?)`, [username]
  );
  if (existing) {
    return res.status(409).json({ error: `El usuario "${username}" ya existe. Elige otro nombre.` });
  }

  const rolIdFinal = resolverRolId(req.body, null);
  ...
  if (Object.keys(permisosOverride).length > 0) {
    try {
      run(
        `UPDATE usuarios SET permisos_override = ? WHERE id = ?`,
...
router.patch('/:id', asyncHandler(async (req, res) => {
  ...
  if (req.body.rol_id !== undefined || req.body.rol !== undefined) {
    const rolIdFinal = resolverRolId(req.body, user.rol_id);
    updates.push(`rol_id = ?`);
  ...
  values.push(userId);
  run(`UPDATE usuarios SET ${updates.join(', ')} WHERE id = ?`, values);
...
router.delete('/:id', asyncHandler(async (req, res) => {
  ...
  run(`UPDATE usuarios SET activo = 0 WHERE id = ?`, [userId]);
