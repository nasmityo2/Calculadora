...
(function () {
  ...
  /* ─── MODO RED (multi-cajero) ─── */
  function renderModoRedInfo(data) {
    var info = document.getElementById('modo-red-info');
    if (!info) return;
    if (!data || !data.activo) {
    ...
    var html = '<p><strong>Modo red activo.</strong> Los cajeros secundarios pueden conectarse desde:</p>';
    var urls = data.urls || [];
    if (urls.length === 0) {
      html += '<p>No se detectaron direcciones IP de red local en este equipo.</p>';
    } else {
      html += '<ul style="margin:0.4rem 0 0.6rem 1.1rem;">';
      for (var i = 0; i < urls.length; i++) {
        html += '<li><code>' + esc(urls[i]) + '</code></li>';
      }
      html += '</ul>';
    }
    if (data.qr) {
      html +=
        '<div style="margin-top:0.5rem;"><img src="' + data.qr +
        '" alt="QR para cajeros secundarios" width="180" height="180" ' +
        'style="border:1px solid var(--border-primary);border-radius:4px;background:#fff;padding:6px;" /></div>' +
        '<p style="margin-top:0.35rem;">Escanea el QR o abre la URL en Chromium para iniciar sesión como cajero.</p>';
    }
    info.className = 'cfg-respaldo-status cfg-respaldo-status--ok';
    info.style.display = 'block';
    info.innerHTML = html;
  }

  function cargarModoRed() {
    apiFetch('/api/configuracion/modo-red')
      .then(function (r) { return r.ok ? r.json() : null; })
      .then(function (data) {
        if (!data) return;
        var chk = document.getElementById('cfg-modo-red-activo');
        if (chk) chk.checked = !!data.activo;
        renderModoRedInfo(data);
      })
      .catch(function () {});
  }

  function guardarModoRed() {
    var chk = document.getElementById('cfg-modo-red-activo');
    var btn = document.getElementById('btn-guardar-modo-red');
    if (!chk) return;
    if (btn) { btn.disabled = true; btn.textContent = 'Guardando...'; }
    apiFetch('/api/configuracion/modo-red', {
      method: 'PATCH',
      ...
      toast(data.mensaje || 'Modo red guardado', data.activo ? 'warning' : 'success');
      cargarModoRed();
    }).catch(function (e) {
      toast(e.message || 'No se pudo guardar el modo red', 'error');
    }).finally(function () {
      if (btn) { btn.disabled = false; btn.textContent = 'Guardar modo red'; }
    });
  }
    ...
    mount: function (host) {
      ...
      host.querySelectorAll('.cfg-tab').forEach(function (tab) {
        tab.addEventListener('click', function () {
          ...
          if (target === 'respaldo') { cargarEstadoRespaldo(); cargarModoRed(); }
      ...
      // Modo red (multi-cajero)
      var btnModoRed = host.querySelector('#btn-guardar-modo-red');
      if (btnModoRed) btnModoRed.addEventListener('click', guardarModoRed);
