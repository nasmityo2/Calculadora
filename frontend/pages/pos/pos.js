...
(function () {
  ...
  function mountPos(host) {
    ...
    var state = {
      ...
      cajaAbierta: false,
      /** false hasta que /api/caja/sesion-activa responda al menos una vez.
       *  Evita el banner falso de «Caja cerrada» mientras la consulta está en vuelo. */
      cajaEstadoConocido: false,
    ...
    function refreshSesionCaja() {
      return apiFetch('/api/caja/sesion-activa')
        ...
        .then(function (d) {
          // El backend SQLite responde { abierta: boolean, sesion: objeto|null }.
          // Validamos el id > 0 porque better-sqlite3 entrega INTEGER nativo.
    ...
    function apiFetch(path, init) {
      ...
      return req.then(function (res) {
        if (res.status === 503) {
          showToast('Base de datos no disponible u ocupada. Intenta de nuevo en unos segundos.', 'danger');
        }
        if (res.status === 401) {
    ...
    function cartTieneMontoFacturable(totalsOptional) {
      if (!state.cart.length) return false;
      var t = totalsOptional || cartTotals();
      return (
        (Number(t.totalUsd) || 0) > 0 ||
        (Number(t.totalBsBcv) || 0) > 0 ||
        (Number(t.totalUsdBcvRef) || 0) > 0
      );
    }
    ...
    function applyCajaUiGate() {
      ...
      var cartOk = cartTieneMontoFacturable();
      var btn = host.querySelector('[data-pos-cobrar]');
      var banner = host.querySelector('[data-pos-caja-banner]');
      if (btn) {
        btn.disabled = !tasasOk || !cartOk;
        if (!tasasOk) {
          btn.title = mensajeTasasNoDisponibles();
        } else if (!cartOk) {
          btn.title = 'Agrega artículos al carrito para cobrar.';
        } else if (!abierta) {
          btn.title =
            'Caja cerrada: al pulsar COBRAR se solicitará abrir la sesión de caja.';
        } else {
          btn.title = '';
        }
      }
      ...
      if (abierta || !state.cajaEstadoConocido) {
        // Caja abierta, o estado aún no confirmado por el servidor:
        // no afirmar «Caja cerrada» sin evidencia (evita la alerta falsa).
        banner.hidden = true;
        banner.innerHTML = '';
        return;
      }
    ...
    function onNexusSessionPos(ev) {
      ...
      if (!u) {
        state.cajaAbierta = false;
        state.sesionCajaId = null;
        state.cajaEstadoConocido = false; // sesión cerrada: estado de caja desconocido
        applyCajaUiGate();
        return;
      }
    ...
    function onVisOrFocusRefreshCaja() {
      ...
      if (
        ...
        state.cajaAbierta = false;
        state.sesionCajaId = null;
        state.cajaEstadoConocido = false;
        applyCajaUiGate();
        return;
      }
      void refreshSesionCaja();
    ...
    function renderCart() {
      ...
      if (!state.cart.length) {
        ...
        tr0.innerHTML =
          '<td colspan="6" class="pos-cart-empty">Sin artículos · Usa <strong>F5</strong> o el campo de búsqueda para agregar.</td>';
      ...
      state.cart.forEach(function (line) {
        ...
        // Descripción: nombre + código en la misma celda
        var cod = line.codigo_barras || line.codigo_interno || '';
        var tdNom = document.createElement('td');
        tdNom.innerHTML =
          '<div class="pos-cart-name-cell">'
          + '<span class="pos-cart-line-name">' + escapeHtml(line.nombre) + '</span>'
          + (cod ? '<span class="pos-cart-line-code">' + escapeHtml(cod) + '</span>' : '')
          + '</div>';
    ...
    function renderTotals() {
      ...
      // Footer total BCV (panel orden: elemento data-pos-total-bs-bcv-footer)
      var elFooterBcv = host.querySelector('[data-pos-total-bs-bcv-footer]');
      if (elFooterBcv)
        elFooterBcv.textContent = 'Bs.\u00A0' + formatBs(t.totalBsBcv);
      updateClassicMarquee();
    ...
    var onTasas = function (e) {
      ...
      if (bcv > 0 && usd > 0) {
        tasasHidratadasOk = true;
      }
      if (Math.abs(bcv - _ultimaTasaBcvPos) < 0.00005 && Math.abs(usd - _ultimaTasaUsdPos) < 0.00005) {
        applyCajaUiGate();
        return;
      }
    ...
    /* ─── MODO COBRO — transición de layout inline ──────────────────── */

    function isCobroModeActive() {
      var pagoPanel = document.getElementById('pos-pago-panel');
      return !!(pagoPanel && pagoPanel.style.display === 'flex');
    }

    var handleCobroKeydown = function (e) {
      if (e.key === 'Escape') {
        e.preventDefault();
        exitCobroMode();
      }
    ...
    function enterCobroMode() {
      var grid = document.getElementById('pos-main-grid');
      if (grid) grid.classList.add('pos-mode-cobro');
      var catalogPanel = document.getElementById('pos-catalog-panel');
      if (catalogPanel) catalogPanel.style.display = 'none';
      var cartCompact = document.getElementById('pos-cart-compact');
      if (cartCompact) {
        cartCompact.style.display = 'flex';
        renderCartCompacto(cartCompact);
      }
      var orderPanel = document.getElementById('pos-order-panel');
      if (orderPanel) orderPanel.style.display = 'none';
      var pagoPanel = document.getElementById('pos-pago-panel');
      if (pagoPanel) pagoPanel.style.display = 'flex';
      document.addEventListener('keydown', handleCobroKeydown);
    }

    function exitCobroMode() {
      var grid = document.getElementById('pos-main-grid');
      if (grid) grid.classList.remove('pos-mode-cobro');
      var catalogPanel = document.getElementById('pos-catalog-panel');
      if (catalogPanel) catalogPanel.style.display = '';
      var cartCompact = document.getElementById('pos-cart-compact');
      if (cartCompact) cartCompact.style.display = 'none';
      var orderPanel = document.getElementById('pos-order-panel');
      if (orderPanel) orderPanel.style.display = '';
      var pagoPanel = document.getElementById('pos-pago-panel');
      if (pagoPanel) pagoPanel.style.display = 'none';
      document.removeEventListener('keydown', handleCobroKeydown);
    }
    ...
    function renderCartCompacto(container) {
      if (!container) return;
      var t = cartTotals();
      var modoMulti = posModoMoneda() !== 'solo_bcv';
      var html = '<div class="pos-cart-compact-header">'
        + '<span class="pos-cart-compact-title">Resumen del pedido</span>'
        + '<button type="button" class="pos-cart-compact-close" id="pos-cart-compact-esc">✕ ESC</button>'
        + '</div>';
      html += '<div class="pos-cart-compact-items">';
      if (!state.cart.length) {
        html += '<div class="pos-compact-item"><span style="color:var(--text-muted);font-size:11px">Carrito vacío</span></div>';
      } else {
        state.cart.forEach(function (line) {
          var qty = Number(line.cantidad) || 1;
          var disc = Number(line.descuento_pct) || 0;
          var precioLinBs = line.precio_bs_bcv || 0;
          var precioLinRef = line.precio_usd_bcv || 0;
          html += '<div class="pos-compact-item">'
            + '<div class="pos-compact-item-name">'
            + escapeHtml(line.nombre)
            + '<span class="pos-compact-item-meta">×' + qty
            + (disc > 0 ? ' · desc.\u00A0' + disc + '%' : '')
            + '</span>'
            + '</div>'
            + '<div class="pos-compact-item-prices">'
            + '<span class="pos-compact-item-bs">Bs.\u00A0' + formatBs(precioLinBs) + '</span>'
            + (modoMulti ? '<span class="pos-compact-item-ref">Ref.\u00A0$' + formatRefUsdBcv(precioLinRef) + '</span>' : '')
            + '</div>'
            + '</div>';
        });
      }
      html += '</div>';
      html += '<div class="pos-compact-separator"></div>';
      html += '<div class="pos-compact-totales">'
        + '<div class="pos-compact-total-row">'
        + '<span>Subtotal</span>'
        + '<span>Bs.\u00A0' + formatBs(t.totalBsBcv) + '</span>'
        + '</div>'
        + '<div class="pos-compact-total-main">Total BCV: Bs.\u00A0' + formatBs(t.totalBsBcv) + '</div>'
        + (modoMulti
          ? '<div class="pos-compact-total-ref">Ref.\u00A0$' + formatRefUsdBcv(t.totalUsdBcvRef) + ' BCV\u00A0·\u00A0USD\u00A0$' + (t.totalUsd || 0).toFixed(2) + '</div>'
          : '')
        + '</div>';
      container.innerHTML = html;
      var closeBtn = container.querySelector('#pos-cart-compact-esc');
      if (closeBtn) {
    ...
    function openCobroModal() {
      ...
      var totals = cartTotals();
      if (!cartTieneMontoFacturable(totals)) {
        showToast('El carrito está vacío', 'warning');
        return;
      }
    ...
    loadSuspendedList();
    void syncPreciosServidorHydrateThenRefresh().then(function (r) {
      // Sesión expirada durante la hidratación: handle401() ya limpió la sesión
      // y está redirigiendo al login — no continuar el arranque del POS.
      if (r && r.unauthorized) return;
      // refreshSesionCaja() y loadImpuestoIvaVentas() resuelven siempre (catch interno);
      // el gate y el render inicial se aplican cuando AMBOS estados reales llegaron.
      return Promise.all([refreshSesionCaja(), loadImpuestoIvaVentas()]).then(function () {
        applyCajaUiGate();
        refreshAll();
